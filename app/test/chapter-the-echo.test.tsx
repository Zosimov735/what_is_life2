/**
 * The Echo's chrome, read structurally.
 *
 * The chapter's done-when is that a player solves it through observation rather
 * than explanatory text, so what the surface is allowed to say is exactly what
 * every other chapter's surface says: one objective line of at most six words,
 * one optional control beside it, and nothing that covers the field or has to
 * be dismissed. That is a structural rule, so it is read structurally, over
 * every objective of the chapter and over every state the sequence can put the
 * surface into — the recoverable setback its ring stands in included.
 *
...
import chapter from '../../content/chapters/the_echo.json';
...
test('the setback the ring stands in is legible without colour', () => {
  const held = 'objective.the_echo.hold_the_ring';
...
test('the chapter names no state outside the closed set', () => {
  // The four the sequence can put the line into. A state outside them would be
  // a surface this file has never rendered.
