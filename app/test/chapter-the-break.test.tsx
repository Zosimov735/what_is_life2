/**
 * The Break's chrome, read structurally.
 *
 * The chapter takes the player's own most-used Route away half way through and
 * asks them to keep operating, so the states its sequence puts the surface into
 * are the interesting ones: the recoverable setback the stranded junction
 * stands in, and the final challenge the break lands inside. What the surface is
 * allowed to say is exactly what every other chapter's surface says — one
 * objective line of at most six words, one optional control beside it, and
 * nothing that covers the field or has to be dismissed.
 *
...
import chapter from '../../content/chapters/the_break.json';
...
test('the final challenge explains the three answers only when it is asked to', () => {
  const last = SEQUENCE[SEQUENCE.length - 1];
  expect(last).toBe('objective.the_break.hold_past_the_break');
...
test('the setback the stranded junction stands in is legible without colour', () => {
  const held = 'objective.the_break.hold_the_run';
