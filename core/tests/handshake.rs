...
fn disagreement_yields_the_protocol_envelope() {
    let refused = Session::new("{\"protocol\":2,\"save_version\":1}")
        .err()
        .expect("a version the core does not speak is refused");
    assert_eq!(
        refused,
        "{\"code\":\"protocol\",\"detail\":{\"protocol\":1,\"save_version\":1},\
         \"message_key\":null}"
    );

    assert!(Session::new("{}").is_err(), "a missing version is refused");
    assert!(
        Session::new("{\"protocol\":1,\"save_version\":2}").is_err(),
    ...
    assert_eq!(
        ...
        format!(
            "{{\"body\":{{\"branch_nonce\":0,\"chapter_index\":0,\"content_changed\":false,\
             \"content_hash\":\"{}\",\
             \"protocol\":1,\"run_id\":\"{KEY}\",\"save_version\":1,\"step\":0,\
